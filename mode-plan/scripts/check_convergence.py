...
def load_round(path):
    ...
    return {
        "score": float(stats.get("score_convergence", 0)),
...
def decide(rounds, max_iterations, threshold, plateau_delta):
    ...
    if n_rounds >= 2:
        prev = rounds[-2]
        delta = abs(last["score"] - prev["score"])
        if delta < plateau_delta:
            return {
                "decision": "PLATEAU",
                "reason": (
                    f"score moved by {delta:.2f} between round {n_rounds-1} "
                    f"({prev['score']:.1f}) and round {n_rounds} "
                    f"({last['score']:.1f}), below plateau delta {plateau_delta}"
                ),
                "rounds_run": n_rounds,
                "final_score": last["score"],
            }
