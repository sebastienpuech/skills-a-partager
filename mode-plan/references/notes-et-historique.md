...
- **v4.5** (2026-09-05) — **La recette a un lecteur et des formats, le plan lit ses leçons** (incident source : repo `un dépôt pro`, chantier `un chantier pro`, 30/08 → 05/09, 7 jours pour 3 estimés ; fiches `lecons/2026-09-05-*.md`, rangées dans le socle `claude-config/lecons/`).
  ...
  Gate C16 étendue dans `self_diagnosis.py` (lecteur + formats dans §10ter) ; off par défaut → evals inchangées, 23/23. 06/09 : gabarit §10ter complété (lecteur, cadence, formats) ; 2 cas d'eval C16 v4.5 (`sd_arret_ok`, `sd_arret_lecteur_formats_missing`) → 25/25 ; `lecons_index.py` livré côté `claude-config` (commit d22957d, 8 fiches indexées au 06/09).
