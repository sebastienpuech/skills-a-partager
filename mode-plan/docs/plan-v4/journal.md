...
- **V2 item ③ — déclencheur cron : LIVRÉ (non enregistré).** Opérationnalise la boucle d'auto-amélioration en passe hebdo. `auto_improve.py --log` (métadonnées → `interactions.jsonl`) + wrapper `auto_improve_cron.ps1` + installeur `register_cron.ps1` (tâche utilisateur, dimanche 03:00, trigger temporel + StartWhenAvailable = survit à la veille) + doc `CRON.md`. Pas de gate (c'est un trigger, pas une capacité). NON enregistré sur la machine (changement système, à lancer par le mainteneur). V1 : la passe mesure + log, ne commit rien (l'étape LLM de patch reste séparée).
...
- **Non enregistré** : l'installation réelle (`register_cron.ps1`) est un changement système → laissée à le mainteneur. Vérifs : `auto_improve --log` écrit bien la ligne ; `run_evals` 20/20.

### 2026-07-02 — Anti-régression de la boucle : prouvée (logique) + enforcée (revert)
- **Question (mainteneur)** : comment vérifie-t-on que l'auto-amélioration ne génère pas de régression ?
